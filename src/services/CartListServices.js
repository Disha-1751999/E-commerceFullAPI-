...
export const CartListService = async (req) => {
    try {
        ...
        let data=await ProductCartModel.aggregate([
            matchStage,
            JoinStageProduct,
            unwindProductStage,
            JoinStageBrand,
...
export const SaveCartListService = async (req) => {
    try {
        ...
        await ProductCartModel.create(reqBody);
...
export const UpdateCartListService = async (req) => {
    try {
        ...
        let reqBody=req.body;
        await  ProductCartModel.updateOne({_id:cartID,userID:user_id},{$set:reqBody});
...
export const RemoveCartListService = async (req) => {
    try {
        ...
        let reqBody=req.body;
        reqBody.userID=user_id;
        await ProductCartModel.deleteOne(reqBody);
